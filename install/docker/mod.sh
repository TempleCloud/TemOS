...
alias docker-moby='screen ~/Library/Containers/com.docker.docker/Data/com.docker.driver.amd64-linux/tty'
...
#
function tms::docker_container_clean_exited() {
    local targets=$@
...
#
function tms::docker_container_exists() {
...
#
function tms::docker_container_running() {
    local container_name=$1
    if [[ ! -z "${container_name}" ]]; then
...
# TODO: Add a timeout!
#
function tms::docker_container_waitfor() {
    local container_name=$1
    if [[ ! -z "${container_name}" ]]; then
