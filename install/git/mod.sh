...
function tms::git_remove_branch() {
    local branch_name=$1
...
function tms::git_remove_local_merged_branches() {
    local filter=$1
    local force_level="-d"
    if [[ -z ${filter} ]]; then
...
alias git-push-info="tms::git_push_info"
alias git-delete-branch="tms::git_remove_branch"